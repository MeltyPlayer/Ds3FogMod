    ...
    public CheckRecord Check(RandomizerOptions opt, Graph g, string start) {
      ...
      foreach (Node node in graph.Values) {
        foreach (Edge edge in node.To) {
          if (edge.Link == null)
            throw new Exception($"Unlinked {edge} leaving {node.Area}");
        }
        foreach (Edge edge in node.From) {
          if (edge.Link == null)
            throw new Exception($"Unlinked {edge} entering {node.Area}");
        }
      }
      ...
      Dictionary<string, NodeRecord>
          recs = new Dictionary<string, NodeRecord>();
      visit(start);
      recs[start] = new NodeRecord {Area = start, Dist = 0};
      bool lastLoop = false;
      Dictionary<Edge, float> duplicatePaths = new Dictionary<Edge, float>();
      Dictionary<string, int> areaCost =
          graph.Values.ToDictionary(n => n.Area, n => n.Cost);
      Dictionary<string, float> extraAreaCost =
          graph.Values.ToDictionary(n => n.Area, n => 0f);

      List<string> getAreas(string dep) {
        return g.ItemAreas.TryGetValue(dep, out List<string> itemAreas)
                   ? itemAreas
                   : new List<string> {dep};
      }
      ...
      bool getDepRecord(string dep, out NodeRecord nr) {
        ...
        if (g.ItemAreas.ContainsKey(dep)) {
          ...
          if (itemAreas.Count == 1)
            return recs.TryGetValue(itemAreas[0], out nr);
      ...
      float addEdge(Edge e, string scalingBase) {
        if (!recs.TryGetValue(e.From, out NodeRecord from))
          throw new Exception($"Internal error: {e} cannot calculate distance");
        ...
        if (e.LinkedExpr == null) {
          ...
          List<string> depVisit =
              e.LinkedExpr
               .Cost(area => getDepRecord(area, out NodeRecord nr)
                                 ? nr.Dist
                                 : 10000000)
               .Item1;
          ...
          foreach (string dep in depVisit) {
            if (!getDepRecord(dep, out NodeRecord nr))
              throw new Exception($"Dependency in edge {e} not visited: {dep}");
        ...
        float candDist = visitedCand
                         .Select(c => areaCost[c] + extraAreaCost[c])
                         .Sum();
        if ((to.Dist == 0 && to.Area != start) || candDist < to.Dist) {
          to.Dist = candDist;
          to.Visited = visitedCand;
          if (scalingBase != null) {
            if (!recs.TryGetValue(scalingBase, out NodeRecord scale))
              throw new Exception(
                  $"Internal error: {e} can't find scaling base {scale}");
            ...
            to.Dist = scaleDist;
            Console.WriteLine(
                $"TTT Setting scaling base of {to.Area} to {scalingBase} {scaleDist}");
          }
        }
        return candDist;
      }
      ...
      while (true) {
        ...
        foreach (string loc in visited.OrderBy(l => recs[l].Dist).ToList()) {
          ...
          foreach (Edge edge in node.To) {
            ...
            if (seenEdge.Contains(edge) || seenEdge.Contains(edge.Link.Pair))
              continue;
            if (edge.LinkedExpr != null) {
              Expr simp = edge.LinkedExpr.Substitute(config).Simplify();
              if (!simp.IsTrue()) {
                if (lastLoop && opt["explain"])
                  Console.WriteLine(
                      $"Missing: {loc} -> {edge.To}. Condition {edge.LinkedExpr} -> {simp}");
                continue;
              }
            }
            Node toNode = graph[edge.To];
            if (toNode.ScalingBase != null &&
                !visited.Contains(toNode.ScalingBase)) {
              if (lastLoop && opt["explain"])
                Console.WriteLine(
                    $"Missing: {loc} -> {edge.To}. Condition {toNode.ScalingBase} for scaling");
              continue;
            }
            ...
            if (visited.Contains(edge.To)) {
              if (edge.Name != null && opt["explain"])
                Console.WriteLine(
                    $"    Additional path: {loc} -> {edge.To} ({dist}) - {edge.Text} -> {(edge.Link == null ? "" : edge.Link.Text)}");
            } else {
              visit(edge.To);
              if (opt["explain"])
                Console.WriteLine(
                    $"{loc} -> {edge.To} ({dist}) ({edge.Text} -> {(edge.Link == null ? "" : edge.Link.Text)}{(edge.LinkedExpr == null ? "" : $", if {edge.LinkedExpr}")})");
            }
          }
        }
        if (lastLoop) {
      ...
      List<string> unvisited = graph
                               .Keys.Except(visited)
                               .Except(
                                   g.Areas.Values
                                    .Where(a => a.HasTag("optional"))
                                    .Select(a => a.Name))
                               .ToList();

      if (opt["explain"])
        Console.WriteLine($"Not visited: [{string.Join(", ", unvisited)}]");
      if (opt["explain"]) Console.WriteLine();
