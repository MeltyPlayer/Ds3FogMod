...
namespace FogMod {
  public class Graph {
    ...
    public (Graph.Edge, Graph.Edge) AddNode(
      ...
      string str = e == null
                       ? side.Text ??
                         (side.HasTag("hard") ? "hard skip" : "in map")
                       : e.Text;
    ...
    public void Connect(Graph.Edge exit, Graph.Edge entrance) {
      if (exit.To != null ||
          entrance.From != null ||
          (exit.Link != null || entrance.Link != null))
    ...
    public void Disconnect(Graph.Edge exit, bool forPair = false) {
      ...
      if (link == null)
        throw new Exception(string.Format("Can't disconnect {0}{1}",
                                          (object) exit,
                                          forPair
                                              ? (object) " as pair"
                                              : (object) ""));
      ...
      if (forPair ||
          exit.Pair == null ||
          (link.Pair == null || exit.Pair == link))
        return;
      this.Disconnect(link.Pair, true);
    }

    public void SwapConnectedEdges(
        Graph.Edge oldExitEdge,
        Graph.Edge newEntranceEdge) {
        ...
        if (link2.Pair != null) {
          ...
          if (exit.Pair == null)
            throw new Exception(string.Format(
                                    "Bad seed: Can't find edge to self-link to reach {0}",
                                    (object) entrance));
    ...
    public void SwapConnectedAreas(string name1, string name2) {
      ...
      for (int index1 = 0; index1 <= 1; ++index1) {
        ...
        List<Graph.Edge> list1 = node1
                                 .From.Where<Graph.Edge>(
                                     (Func<Graph.Edge, bool>)
                                     (e => !e.IsFixed &&
                                           e.Pair == null == unpaired))
                                 .ToList<Graph.Edge>();
        List<Graph.Edge> list2 = node2
                                 .From.Where<Graph.Edge>(
                                     (Func<Graph.Edge, bool>)
                                     (e => !e.IsFixed &&
                                           e.Pair == null == unpaired))
        ...
        list2.Reverse();
        for (int index2 = 0;
             index2 < Math.Min(list1.Count, list2.Count);
             ++index2)
    ...
    public void Construct(RandomizerOptions opt, AnnotationData ann) {
      this.Areas =
          ann.Areas.ToDictionary<AnnotationData.Area, string,
              AnnotationData.Area>(
              (Func<AnnotationData.Area, string>) (a => a.Name),
              (Func<AnnotationData.Area, AnnotationData.Area>) (a => a));
      this.ItemAreas =
          ann.KeyItems.ToDictionary<AnnotationData.Item, string, List<string>>(
              (Func<AnnotationData.Item, string>) (item => item.Name),
              (Func<AnnotationData.Item, List<string>>)
      ...
      foreach (AnnotationData.Entrance entrance in ann
                                                   .Entrances
      ...
      foreach (AnnotationData.Entrance warp in ann.Warps) {
        if (!warp.HasTag("unused") &&
            (warp.ASide == null || warp.BSide == null))
          throw new Exception(warp.FullName + " warp missing both sides");
      }
      Dictionary<string, List<string>> dictionary1 =
        ...
        if (!entrance.HasTag("unused")) {
          if (opt.Game == GameSpec.FromGame.DS3) {
            if (entrance.HasTag("norandom"))
              ...
              entrance.IsFixed = true;
              if (opt["dumptext"])
                Util.AddMulti<string, string>(
                    (IDictionary<string, List<string>>) dictionary1,
                    "boss",
                    entrance.Text);
      ...
      foreach (AnnotationData.Entrance entrance in ann
        ...
        foreach (AnnotationData.Side side in entrance.Sides()) {
          if (!this.Areas.ContainsKey(side.Area))
            throw new Exception(entrance.FullName +
                                " goes to nonexistent " +
                                side.Area);
          side.Expr = getExpr(side.Cond);
          if (!entrance.IsFixed &&
              side.ExcludeIfRandomized != null &&
              !this.EntranceIds[side.ExcludeIfRandomized].IsFixed)
            this.Ignore.Add((entrance.FullName, side.Area));
        }
      }
      this.Nodes =
          this.Areas.ToDictionary<KeyValuePair<string, AnnotationData.Area>,
              string, Graph.Node>(
              (Func<KeyValuePair<string, AnnotationData.Area>, string>)
              (e => e.Key),
              (Func<KeyValuePair<string, AnnotationData.Area>, Graph.Node>)
              (e => new Graph.Node() {
                    Area = e.Key,
                    Cost = areaCost(e.Value),
                    ScalingBase =
                        opt["dumpdist"] || opt["scalingbase"]
                            ? e.Value.ScalingBase
                            : (string) null
                }));
      foreach (AnnotationData.Area area in ann.Areas) {
        if (area.To != null) {
          foreach (AnnotationData.Side side1 in area.To) {
            if (!side1.HasTag("temp") &&
                (!side1.HasTag("hard") || opt["hard"]) &&
                ((!side1.HasTag("treeskip") || opt["treeskip"]) &&
                 (!side1.HasTag("instawarp") || opt["instawarp"]))) {
      ...
      Dictionary<Graph.Connection, List<(Graph.Edge, Graph.Edge)>> dictionary2 =
          new Dictionary<Graph.Connection, List<(Graph.Edge, Graph.Edge)>>();
      foreach (AnnotationData.Entrance warp in ann.Warps) {
        if (!warp.ASide.HasTag("temp") && !warp.HasTag("unused")) {
          Graph.Edge exit = this.AddNode(warp.ASide, warp, true, false).Item1;
          Graph.Edge entrance =
              this.AddNode(warp.BSide, warp, false, true).Item2;
          if (warp.IsFixed)
            this.Connect(exit, entrance);
          else if (!warp.HasTag("unique"))
            Util.AddMulti<Graph.Connection, (Graph.Edge, Graph.Edge)>(
                (IDictionary<Graph.Connection, List<(Graph.Edge, Graph.Edge)>>)
                dictionary2,
                new Graph.Connection(warp.ASide.Area, warp.BSide.Area),
                (exit, entrance));
        }
      }
      foreach (
          KeyValuePair<Graph.Connection, List<(Graph.Edge, Graph.Edge)>>
              keyValuePair in dictionary2) {
        if (keyValuePair.Value.Count != 2)
          throw new Exception(string.Format(
                                  "Bidirectional warp expected for {0} - non-bidirectional should be marked unique",
        ...
        if (!opt["unconnected"]) {
          ...
          if (edge17.From == edge19.From)
            throw new Exception(string.Format(
                                    "Duplicate warp {0} and {1} - should be marked unique",
                                    (object) edge17,
                                    (object) edge19));
          if (edge17.From != edge20.To)
            throw new Exception(string.Format(
                                    "Internal error: warp {0} and {1} not equivalent",
                                    (object) edge17,
                                    (object) edge20));
          edge17.Pair = edge20;
          edge20.Pair = edge17;
          ...
          if (sideList.Count == 1) {
            ...
            if (entrance1.HasTag("door")) {
              Graph.Connection connection =
                  new Graph.Connection(side1.Area, side2.Area);
              if (!connectionSet.Contains(connection)) {
                ...
                if (side1.Expr != null || side2.Expr != null)
                  throw new Exception(string.Format(
                                          "Door cond {0} and cond {1} {2} together for {3}",
                                          (object) expr1,
                                          (object) side1.Expr,
                                          (object) side2.Expr,
                                          (object) entrance1.FullName));
                AnnotationData.Side side3 = side1;
                AnnotationData.Expr expr2;
                if (!side1.HasTag("dnofts"))
                  expr2 = expr1;
                else if (expr1 != null)
                  expr2 = new AnnotationData.Expr(
                      new List<AnnotationData.Expr>() {
                          expr1,
                ...
                side4.Expr = expr3;
                this.Connect(this.AddNode(side1, entrance1, true, true).Item1,
                             this.AddNode(side2, entrance1, true, true).Item2);
              }
            } else if (entrance1.IsFixed || !opt["unconnected"]) {
              Graph.Edge exit =
                  this.Ignore.Contains((entrance1.FullName, side2.Area))
                      ? (Graph.Edge) null
                      : this.AddNode(side2, entrance1, true, true).Item1;
              Graph.Edge entrance2 =
                  this.Ignore.Contains((entrance1.FullName, side1.Area))
                      ? (Graph.Edge) null
                      : this.AddNode(side1, entrance1, true, true).Item2;
      ...
      AnnotationData.Expr getExpr(string cond) {
        ...
        foreach (string freeVar in expr.FreeVars()) {
          if (!this.Areas.ContainsKey(freeVar) &&
              !this.ItemAreas.ContainsKey(freeVar))
    ...
    public class Edge {
      ...
      public Graph.Edge Pair {
        ...
        set {
          if (value != null && this.Type == value.Type)
            throw new Exception(string.Format("Cannot pair {0} and {1}",
                                              (object) this,
                                              (object) value));
          this.pair = value;
        }
      ...
      public Graph.Edge Link {
        ...
        set {
          if (value != null && this.Type == value.Type)
            throw new Exception(string.Format("Cannot link {0} and {1}",
                                              (object) this,
                                              (object) value));
          this.link = value;
        }
      ...
      public override string ToString() {
        return string.Format("Edge[Name={0}, From={1}, To={2}, Expr={3}]",
                             (object) this.Name,
                             (object) this.From,
                             (object) this.To,
                             (object) this.Expr);
