...
namespace FogMod
{
  public static class Program
  {
    ...
    {
      if (args.Length != 0 && !((IEnumerable<string>) args).Contains<string>("/gui"))
      {
        ...
        GameSpec.FromGame fromGame = ((IEnumerable<string>) args).Contains<string>("ptde") ? GameSpec.FromGame.DS1 : GameSpec.FromGame.DS1R;
        GameSpec.FromGame game = GameSpec.FromGame.DS3;
        opt.Game = game;
        string gameDir = GameSpec.ForGame(game).GameDir;
        if (game == GameSpec.FromGame.DS3)
          new Randomizer().Randomize(opt, game, opt["mergemods"] ? gameDir + "\\randomizer" : (string) null, gameDir + "\\fog");
        else
