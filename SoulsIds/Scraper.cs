...
namespace SoulsIds {
  public class Scraper {
    public static readonly Dictionary<string, Universe.Namespace> MsgTypes =
        new Dictionary<string, Universe.Namespace>() {
            {
                "NPC名",
    ...
    public static readonly Dictionary<Universe.Namespace, string> ItemParams =
        new Dictionary<Universe.Namespace, string>() {
            {
                Universe.Namespace.WEAPON,
                "EquipParamWeapon"
            }, {
    ...
    private static readonly Dictionary<GameSpec.FromGame, string> talkParamMsgId
        = new Dictionary<GameSpec.FromGame, string>() {
    ...
    public async Task<bool> ScrapeMsgs(Universe u) {
      ...
      if (Directory.Exists(this.spec.GameDir +
                           "\\" +
                           this.spec.MsgDir +
                           "\\talk")) {
      ...
      foreach (PARAM.Row row in this.Params["TalkParam"].Rows) {
        ...
        if (num > 0) {
          Universe.Obj key =
              Universe.Obj.Of(Universe.Namespace.DIALOGUE, (object) num);
      ...
      if (this.spec.Game == GameSpec.FromGame.DS1R) {
        foreach (PARAM.Row row in this.Params["ItemLotParam"].Rows) {
          ...
          for (int index = 1; index <= 8; ++index) {
            int id2 = (int) row[string.Format("lotItemId0{0}", (object) index)]
                .Value;
            int num =
                (int) row[string.Format("lotItemCategory0{0}", (object) index)]
                    .Value;
            if (id2 != 0 && num != -1)
              u.Add(Universe.Verb.PRODUCES,
                    from,
                    Universe.Obj.Item((uint) num, id2));
          }
        }
        foreach (PARAM.Row row in this.Params["ShopLineupParam"].Rows) {
          if (row.ID < 9000000L) {
            ...
            if (id1 != -1)
              u.Add(Universe.Verb.WRITES,
                    from,
                    Universe.Obj.EventFlag(id1, -1));
            ...
            u.Add(Universe.Verb.PRODUCES,
                  from,
                  Universe.Obj.Item((uint) num, id3));
        ...
        foreach (PARAM.Row row in this.Params["EquipMtrlSetParam"].Rows) {
          ...
          for (int index = 1; index <= 5; ++index) {
            int id = (int) row[string.Format("materialId0{0}", (object) index)]
                .Value;
            if (id > 0)
        ...
        foreach (PARAM.Row row in this.Params["ItemLotParam"].Rows) {
          ...
          for (int index2 = 1; index2 <= 8; ++index2) {
            int id2 = (int) row[string.Format("ItemLotId{0}", (object) index2)]
                .Value;
            uint type =
                (uint) row[
                        string.Format("LotItemCategory0{0}", (object) index2)]
                    .Value;
        ...
        foreach (PARAM.Row row in this.Params["ShopLineupParam"].Rows) {
          if (row.ID < 9000000L) {
            ...
            if (id2 != -1)
              u.Add(Universe.Verb.READS, from, Universe.Obj.EventFlag(id2, -1));
            Universe.Obj index =
                Universe.Obj.Item((uint) (byte) row["equipType"].Value,
    ...
    public void LoadNames(Universe u) {
      if (this.spec.NameDir == null)
        return;
      foreach (KeyValuePair<string, string> loadName in
          this.editor.LoadNames<string>("ModelName",
                                        (Func<string, string>) (n => n),
                                        true)) {
        u.Names[Universe.Obj.Of(Universe.Namespace.OBJ_MODEL,
                                (object) loadName.Key)] = loadName.Value;
        u.Names[Universe.Obj.Of(Universe.Namespace.CHR_MODEL,
                                (object) loadName.Key)] = loadName.Value;
      }
      foreach (KeyValuePair<int, string> loadName in this.editor.LoadNames<int>(
          "CharaInitParam",
          (Func<string, int>
          ) (n => int.Parse(
                  n)),
          true))
        u.Names[Universe.Obj.Of(Universe.Namespace.NPC, (object) loadName.Key)]
            = loadName.Value;
      foreach (KeyValuePair<int, string> loadName in this.editor.LoadNames<int>(
          "ShopQwc",
          (Func<string, int>
                  ...
                  n)),
          true))
        u.Names[Universe.Obj.Of(Universe.Namespace.EVENT_FLAG,
                                (object) loadName.Key)] = loadName.Value;
    }
    ...
    public bool ScrapeMaps(Universe u) {
      ...
      if (this.spec.Game == GameSpec.FromGame.SDT) {
        foreach (KeyValuePair<string, MSBS> keyValuePair in
          ...
          foreach (MSBS.Event.Treasure treasure in msbs.Events.Treasures) {
            if (treasure.TreasurePartName != null && treasure.ItemLotID != -1) {
              ...
              Universe.Obj from2 =
                  Universe.Obj.Part(key, treasure.TreasurePartName);
              u.Add(Universe.Verb.PRODUCES, from2, obj);
              u.Add(Universe.Verb.PRODUCES,
                    obj,
                    Universe.Obj.Lot(treasure.ItemLotID));
            }
            ++index1;
          }
          ...
          foreach (MSBS.Part entry in msbs.Parts.GetEntries()) {
            if (entry is MSBS.Part part) {
              ...
              if (part.EntityID != -1)
                u.Add(Universe.Verb.CONTAINS,
                      Universe.Obj.Entity(part.EntityID),
                      obj);
              foreach (int id in ((IEnumerable<int>) part.EntityGroupIDs)
                  .Where<int>((Func<int, bool>) (groupID => groupID > 0)))
              ...
              if (part is MSBS.Part.Enemy enemy) {
                ...
                if (enemy.NPCParamID != -1)
                  u.Add(Universe.Verb.CONTAINS,
                        obj,
                        Universe.Obj.Npc(enemy.NPCParamID));
      ...
      } else if (this.spec.Game == GameSpec.FromGame.DS1 ||
                 this.spec.Game == GameSpec.FromGame.DS1R) {
        foreach (KeyValuePair<string, MSB1> keyValuePair in
            this.editor.Load<MSB1>(this.spec.MsbDir,
                                   (Func<string, MSB1>)
                                   (path => !path.Contains("m99")
                                                ? SoulsFile<MSB1>.Read(path)
                                                : (MSB1) null),
                                   "*.msb")) {
          string key = keyValuePair.Key;
              ...
              if (entry is MSB1.Part part) {
                ...
                if (part is MSB1.Part.Enemy enemy) {
                  ...
                  if (enemy.NPCParamID != -1 && id == "c0000")
                    u.Add(Universe.Verb.CONTAINS,
                          obj,
                          Universe.Obj.Human(enemy.NPCParamID));
                  if (enemy.TalkID != -1)
                    u.Add(Universe.Verb.CONTAINS,
                          obj,
                          Universe.Obj.Esd(enemy.TalkID));
        ...
        if (this.spec.Game != GameSpec.FromGame.DS3)
          return false;
        foreach (KeyValuePair<string, MSB3> keyValuePair in
            this.editor.Load<MSB3>(this.spec.MsbDir,
                                   (Func<string, MSB3>)
                                   (path => !path.Contains("m99")
                                                ? SoulsFile<MSB3>.Read(path)
                                                : (MSB3) null),
                                   "*.dcx")) {
          string key = keyValuePair.Key;
          Universe.Obj from = Universe.Obj.Map(key);
            ...
            foreach (MSB3.Part entry in msB3.Parts.GetEntries()) {
              if (entry is MSB3.Part part) {
                ...
                if (part.EventEntityID != -1)
                  u.Add(Universe.Verb.CONTAINS,
                        Universe.Obj.Entity(part.EventEntityID),
                        obj);
                if (part is MSB3.Part.Enemy enemy) {
                  ...
                  if (enemy.CharaInitID > 0)
                    u.Add(Universe.Verb.CONTAINS,
                          obj,
                          Universe.Obj.Human(enemy.CharaInitID));
                  if (enemy.TalkID != -1)
                    u.Add(Universe.Verb.CONTAINS,
                          obj,
                          Universe.Obj.Esd(enemy.TalkID));
