...
namespace SoulsIds {
  public class Events {
    private static Dictionary<int, int> ARG_LENGTH_ = new Dictionary<int, int> {
        [0] = 1,
        [1] = 2,
        [2] = 4,
        [3] = 1,
    ...
    private static object ParseArg_(string arg, ArgType type) {
      switch (type) {
        case ArgType.Byte:
    ...
    private static string TextArg_(object arg) {
      return arg is float f
      ...
      foreach (EMEDF.ClassDoc bank in doc_.Classes) {
        foreach (EMEDF.InstrDoc instr in bank.Instructions) {
          ...
          foreach (EMEDF.ArgDoc arg in instr.Arguments) {
            int len = ARG_LENGTH_[(int) arg.Type];
            if (bytePos % len > 0) bytePos += len - (bytePos % len);
            AddMulti_(funcBytePositions_, instr, bytePos);
            bytePos += len;
          }
        }
      }
    ...
    public Instr Parse(
      ...
      EMEDF.InstrDoc instrDoc = doc_[instr.Bank][instr.ID];
      List<ArgType> argTypes = (isInit || instrDoc == null)
                                   ? Enumerable
                                     .Repeat(ArgType.Int32,
    ...
    public class Instr {
      ...
      public override string ToString()
        => $"{Name} ({string.Join(",", Args.Select(a => TextArg_(a)))})";
    }

    private int IndexFromByteOffset_(Instr instr, int offset) {
      int paramIndex = funcBytePositions_[instr.Doc].IndexOf(offset);
      if (paramIndex == -1)
        throw new Exception(
            $"Finding {instr.Name}, target {offset}, available {string.Join(",", funcBytePositions_[instr.Doc])}");
      return paramIndex;
    }
    ...
    public EMEVD.Instruction CopyInstruction(EMEVD.Instruction i) {
      return i.Layer.HasValue
                 ? new EMEVD.Instruction(i.Bank, i.ID, i.Layer.Value, i.ArgData)
                 : new EMEVD.Instruction(i.Bank, i.ID, i.ArgData);
    }
    ...
    public EMEVD.Event CopyEvent(EMEVD.Event src, int newId) {
      ...
      if (src.Parameters.Count > 0) {
        newEvent.Parameters = src.Parameters
                                 .Select(
                                     p => new EMEVD.Parameter(
                                         p.InstructionIndex,
                                         p.TargetStartByte,
                                         p.SourceStartByte,
                                         p.ByteCount))
      ...
      newEvent.Instructions =
          src.Instructions.Select(i => CopyInstruction(i)).ToList();
      return newEvent;
    }
    ...
    public Instr CopyInit(Instr instr, EMEVD.Event newEvent) {
      ...
      if (newInstr.Val.Bank == 2000 && newInstr.Val.ID == 0) {
        if (newEvent == null)
          throw new Exception(
              $"Internal error: Event not provided for copying {string.Join(",", instr.Args)}");
        newInstr[0] = 0;
    ...
    public class OldParams {
      ...
      public Dictionary<EMEVD.Instruction, List<EMEVD.Parameter>>
          newInstructions =
              new Dictionary<EMEVD.Instruction, List<EMEVD.Parameter>>();
      ...
      public void Postprocess() {
        if (Event == null ||
            (Event.Parameters.Count == 0 && newInstructions.Count == 0)) return;
        Dictionary<EMEVD.Instruction, int> currentIndices =
            Event.Instructions.Select((a, i) => (a, i))
                 .ToDictionary(p => p.Item1, p => p.Item2);
        // Update old indices
        Event.Parameters = Event.Parameters.Select(p => {
                                  if (currentIndices.TryGetValue(
                                      Original[(int) p.InstructionIndex],
        ...
        foreach (KeyValuePair<EMEVD.Instruction, List<EMEVD.Parameter>> entry in
            newInstructions) {
          if (!currentIndices.TryGetValue(entry.Key, out int currentIndex))
            continue;
    ...
    public class EventEdits {
      ...
      public Dictionary<(string, string), List<InstrEdit>> NameArgEdits {
        get;
        set;
      }
      ...
      public Dictionary<int, List<InstrEdit>> pendingAdds =
          new Dictionary<int, List<InstrEdit>>();

      // Returns all applicable edits
      public List<InstrEdit> GetMatches(Instr instr) {
        List<InstrEdit> nameEdit = null;
        if (NameEdits != null &&
            !NameEdits.TryGetValue(instr.Name, out nameEdit) &&
            ArgEdits == null &&
            NameArgEdits == null) return null;
        List<string> strArgs = instr.Args.Select(a => TextArg_(a)).ToList();
        List<InstrEdit> edits = new List<InstrEdit>();
        if (ArgEdits != null) {
          edits.AddRange(strArgs.SelectMany(
                             s => ArgEdits.TryGetValue(
                                      s,
                                      out List<InstrEdit> edit)
                                      ? edit
                                      : new List<InstrEdit>()));
        }
        if (nameEdit != null) {
        ...
        if (NameArgEdits != null &&
            NameArgEdits.TryGetValue((instr.Name, string.Join(",", strArgs)),
                                     out List<InstrEdit> nameArgEdit)) {
      ...
      public void ApplyEdits(Instr instr, int index) {
        ...
        edits.ForEach(edit => {
          ...
          if (edit.Add != null) {
            AddMulti_(pendingAdds, index, edit);
          }
          if (!removed) {
            ...
            if (edit.ValEdit != null) {
              for (int i = 0; i < instr.Args.Count; i++) {
                if (edit.ValEdit.TryGetValue(TextArg_(instr[i]),
                                             out string replace)) {
                  instr[i] = replace;
                }
              }
      ...
      public void AddEdit(
          string toFind,
          Predicate<string> docName,
          InstrEdit edit) {
        if (int.TryParse(toFind, out var _)) {
          if (ArgEdits == null)
            ArgEdits = new Dictionary<string, List<InstrEdit>>();
          AddMulti_(ArgEdits, toFind, edit);
        } else if (docName(toFind)) {
          // If this isn't a name, it will come up later as an unused pending edit
          if (NameEdits == null)
            NameEdits = new Dictionary<string, List<InstrEdit>>();
          AddMulti_(NameEdits, toFind, edit);
        } else {
          (string cmd, List<string> addArgs) = ParseCommandString_(toFind);
          if (NameArgEdits == null)
            NameArgEdits = new Dictionary<(string, string), List<InstrEdit>>();
          AddMulti_(NameArgEdits, (cmd, string.Join(",", addArgs)), edit);
        }
      ...
      public void AddReplace(string toFind, string toVal = null) {
        ...
        if (toVal != null || Regex.IsMatch(toFind, @"^\d+\s*->\s*\d+$")) {
          string[] parts = toVal == null
                               ? Regex.Split(toFind, @"\s*->\s*")
                               : new[] {toFind, toVal};
          edit.ValEdit = new Dictionary<string, string>();
          edit.ValEdit[parts[0]] = parts[1];
          if (ArgEdits == null)
            ArgEdits = new Dictionary<string, List<InstrEdit>>();
          AddMulti_(ArgEdits, parts[0], edit);
        } else {
    ...
    public class InstrEdit {
      ...
      public override string ToString()
        => $"Edit (Add {Add}, Remove {Remove}, PosEdit {(PosEdit == null ? "" : string.Join(",", PosEdit))}, ValEdit {(ValEdit == null ? "" : string.Join(",", ValEdit))})";
    }
    ...
    public void AddMacro(EventEdits edits, List<EventAddCommand> adds) {
      foreach (EventAddCommand add in Enumerable.Reverse(adds)) {
        if (add.Before == null && add.After == null) {
          ...
          AddMacro(edits,
                   add.After ?? (add.Before == "start" ? null : add.Before),
                   add.After != null,
                   add.Cmd);
        }
      }
    ...
    public void ApplyAdds(
        EventEdits edits,
        EMEVD.Event e,
        OldParams oldParams = null) {
      // Add all commands in reverse order, to preserve indices
      foreach (KeyValuePair<int, List<InstrEdit>> lineEdit in edits
                                                              .pendingAdds
                                                              .OrderByDescending(
                                                                  item => item
                                                                      .Key)) {
            ...
            if (addEdit.AddParams != null) {
              if (oldParams == null)
                throw new ArgumentException(
                    $"Can't add instruction with parameters if old params cannot be added in {edits}");
    ...
    public void AddMacro(
      ...
      if (toFind == null) {
        ...
        AddMulti_(edits.pendingAdds, addAfter ? -1 : 0, edit);
      } else {
        edits.AddEdit(toFind, n => docByName_.ContainsKey(n), edit);
      }
    }

    public void RemoveMacro(EventEdits edits, string toFind) {
      edits.AddEdit(toFind,
                    n => docByName_.ContainsKey(n),
                    new InstrEdit {
                        Remove = true
                    });
    }
    ...
    public EMEVD.Instruction ParseAdd(string add) {
      (string cmd, List<string> addArgs) = ParseCommandString_(add);
      if (!docByName_.TryGetValue(cmd, out (int, int) docId))
        throw new Exception($"Unrecognized command '{cmd}'");
      EMEDF.InstrDoc addDoc = doc_[docId.Item1][docId.Item2];
      List<ArgType> argTypes = addDoc
                               .Arguments
                               .Select(
                                   arg => arg.Type == 8
                                              ? ArgType.UInt32
      ...
      return new EMEVD.Instruction(docId.Item1,
                                   docId.Item2,
                                   addArgs.Select(
                                       (a, j) => ParseArg_(a, argTypes[j])));
    }
    ...
    public (EMEVD.Instruction, List<EMEVD.Parameter>) ParseAddArg(
      ...
      if (addArgs.Count != argTypes.Count)
        throw new Exception(
            $"Expected {argTypes.Count} arguments for {cmd}, given {addArgs.Count} in {add}");
      List<int> paramOffsets =
          funcBytePositions_.TryGetValue(addDoc, out var val) ? val : null;
      List<object> args = new List<object>();
      List<EMEVD.Parameter> ps = new List<EMEVD.Parameter>();
      for (int i = 0; i < addArgs.Count; i++) {
        string arg = addArgs[i];
        if (arg.StartsWith("X")) {
          List<int> pos = arg.Substring(1)
                             .Split('_')
                             .Select(pv => int.Parse(pv))
                             .ToList();
          if (paramOffsets == null || i >= paramOffsets.Count) {
            throw new Exception(
                $"Can't substitute at pos {i} in {add} (found indices [{(paramOffsets == null ? "none" : string.Join(",", paramOffsets))})])");
          }
          if (pos.Count != 2)
            throw new Exception(
                $"Invalid parameter format at pos {i} of {add}");
          EMEVD.Parameter p =
              new EMEVD.Parameter(addIndex, paramOffsets[i], pos[0], pos[1]);
          ps.Add(p);
          args.Add(ParseArg_("0", argTypes[i]));
        } else {
          args.Add(ParseArg_(arg, argTypes[i]));
        }
      }
    ...
    public List<int> FindCond(EMEVD.Event e, string req) {
      ...
      for (int i = 0; i < e.Instructions.Count; i++) {
        ...
        if (isGroup &&
            instr.Name.StartsWith("IF") &&
            instr[0].ToString() == req) {
          cond.Add(i);
          continue;
        } else if (isGroup &&
                   instr.Name == "IF Condition Group" &&
                   instr[2].ToString() == req) {
          cond.Add(i);
          return cond;
        } else if (!isGroup &&
                   instr.Name == req &&
      ...
      throw new Exception(
          $"Couldn't find ending condition '{req}', group {isGroup}, in event {e.ID}");
    }
    ...
    public List<EMEVD.Instruction> RewriteCondGroup(
      ...
      return after.Select(afterInstr => {
                    ...
                    if (instr.Name == "IF Condition Group") {
                      if (sourceCond == 0)
                        throw new Exception(
                            $"Internal error: can't infer condition group for {instr}");
                      instr[0] = targetCond;
                      ...
                      instr[0] = after.Count == 1
                                     ? targetCond
                                     : (sbyte) (sourceCond > 0 ? 12 : -12);
                    }
    ...
    public class InstructionDebug {
      ...
      public string CallString()
        => $"{Name} ({string.Join(", ", args.Select((a, i) => highlightArgs.Contains(i) ? $"{a}*" : a))})";

      public override string ToString()
        => Caller == null
               ? $"[Event {Event}] {CallString()}"
               : $"{Caller.CallString()} - {CallString()}";
    }

    // Code for producing event configs
    public SortedDictionary<int, EventDebug> GetHighlightedEvents(
        Dictionary<string, EMEVD> emevds,
        HashSet<int> ids,
        Predicate<Instr> alwaysHighlight = null) {
      Dictionary<(int, int), List<InstructionDebug>> argCommands =
          new Dictionary<(int, int), List<InstructionDebug>>();
      SortedDictionary<int, EventDebug> eventInfos =
          new SortedDictionary<int, EventDebug>();
          ...
          for (int i = 0; i < e.Instructions.Count; i++) {
            ...
            foreach (EMEVD.Parameter param in e.Parameters) {
              if (param.InstructionIndex == i) {
                int paramIndex =
                    IndexFromByteOffset_(instr, (int) param.TargetStartByte);
                instr[paramIndex] =
                    $"X{param.SourceStartByte}_{param.ByteCount}";
            ...
            InstructionDebug info = new InstructionDebug {
                Event = (int) e.ID,
                Name = instr.Name,
                args = instr.Doc.Arguments
                            .Select((arg, j) => $"{arg.Name} = {instr[j]}")
                            .ToList(),
                highlightArgs = highlightArgs,
            };
            if (instr.Args.Count > instr.Doc.Arguments.Length)
              info.args.AddRange(instr.Args.Skip(instr.Doc.Arguments.Length)
                                      .Select(arg => arg.ToString()));
            foreach ((int, int) id in usedParams)
              AddMulti_(argCommands, id, info);
      ...
      foreach (KeyValuePair<string, EMEVD> entry in emevds) {
        foreach (EMEVD.Event e in entry.Value.Events) {
          ...
          for (int i = 0; i < e.Instructions.Count; i++) {
            ...
            List<int> entityParams = Enumerable
                                     .Range(0, instr.Args.Count - instr.Offset)
                                     .Where(
                                         j => instr[instr.Offset + j] is int
                                                  argint &&
                                              ids.Contains(argint))
                                     .ToList();
            if (entityParams.Count > 0) {
              ...
              InstructionDebug caller = new InstructionDebug {
                  ...
                  args = instr
                         .Args.Select((a, pos)
                                          => pos >= instr.Offset
                                                 ? $"X{(pos - instr.Offset) * 4}_4 = {a}"
                                                 : $"{a}")
                         .ToList()
              };
              calleeInfo.callers.Add(caller);
              calleeInfo.ids.AddRange(entityParams.Select(
                                          j => instr[instr.Offset + j] is int
                                                   argint
                                                   ? argint
                                                   : 0));
              List<string> paramStrings =
                  entityParams.Select(j => $"X{j * 4}_4").ToList();
              foreach (int j in entityParams) {
                caller.highlightArgs.Add(instr.Offset + j);
                argCommands.TryGetValue((instr.Callee, j * 4),
                                        out List<InstructionDebug> usages);
                if (usages != null) {
    ...
    public List<T> CreateEventConfig<T>(
      ...
      foreach (KeyValuePair<int, EventDebug> entry in eventInfos.OrderBy(
          e => e.Key)) {
        ...
        process = process &&
                  (info.HighlightInstr ||
                   info.ids.Any(id => eligibleFilter(id)));
        if (eventsOverride?.Count > 0)
          process = eventsOverride.Contains(entry.Key);
        else if (idsOverride?.Count > 0)
          process = idsOverride.Intersect(info.ids).Count() > 0;
        if (!process) continue;
        ...
        spec.DebugInit = info.callers.Count > 0
                             ? info.callers.Select(c => c.CallString()).ToList()
                             : null;
        spec.DebugCommands = info.instructions
                                 .Select(
                                     c => $"{(c.highlightArgs.Count > 0 ? "+ " : "")}{c.CallString()}")
                                 .ToList();
        toWrite.Add(spec);
      }
      return toWrite;
    ...
    private static void AddMulti_<K, V>(
