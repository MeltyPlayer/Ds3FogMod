  ...
  public class ESD : SoulsFile<ESD> {
    ...
    protected override void Read(BinaryReaderEx br) {
      ...
      Dictionary<long, long[]> dictionary1 =
          new Dictionary<long, long[]>(capacity1);
      for (int index = 0; index < capacity1; ++index) {
        long key = ESD.ReadVarint(br, this.LongFormat);
        long[] numArray =
            this.ReadStateGroup(br, this.LongFormat, position, (long) num1);
        if (dictionary1.ContainsKey(key))
          throw new FormatException("Duplicate state group ID.");
        dictionary1[key] = numArray;
      }
      Dictionary<long, ESD.State> states1 =
          new Dictionary<long, ESD.State>(capacity2);
      for (int index = 0; index < capacity2; ++index)
        states1[br.Position - position] =
            new ESD.State(br, this.LongFormat, position);
      Dictionary<long, ESD.Condition> conditions =
          new Dictionary<long, ESD.Condition>(capacity3);
      for (int index = 0; index < capacity3; ++index)
        conditions[br.Position - position] =
            new ESD.Condition(br, this.LongFormat, position);
      foreach (ESD.State state in states1.Values)
        state.GetConditions(conditions);
      this.StateGroups =
          new Dictionary<long, Dictionary<long, ESD.State>>(capacity1);
      Dictionary<long, Dictionary<long, long>> dictionary2 =
      ...
      foreach (long key in dictionary1.Keys) {
        ...
        Dictionary<long, ESD.State> states2 =
            this.TakeStates((long) num1, stateOffsets, states1, out stateIDs);
    ...
    protected override void Write(BinaryWriterEx bw) {
      ...
      bw.WriteInt32(count);
      bw.WriteInt32(this.StateGroups.Values.Sum<Dictionary<long, ESD.State>>(
                        (Func<Dictionary<long, ESD.State>, int>)
                        (sg => sg.Count + (sg.Count == 1 ? 0 : 1))));
      ...
      ESD.WriteVarint(bw,
                      this.LongFormat,
                      this.Name == null ? 0L : (long) (this.Name.Length + 1));
      ...
      list.Sort();
      Dictionary<long, List<long>> dictionary1 =
      ...
      if (this.StateGroups.Count == 0) {
        ...
        ESD.FillVarint(bw,
                       this.LongFormat,
                       "StateGroupsOffset",
                       bw.Position - position1);
        foreach (long index in list) {
          ESD.WriteVarint(bw, this.LongFormat, index);
          ESD.ReserveVarint(bw,
                            this.LongFormat,
                            string.Format("StateGroup{0}:StatesOffset1",
                                          (object) index));
          ESD.WriteVarint(bw,
                          this.LongFormat,
                          (long) this.StateGroups[index].Count);
          ESD.ReserveVarint(bw,
                            this.LongFormat,
                            string.Format("StateGroup{0}:StatesOffset2",
                                          (object) index));
        }
      }
      Dictionary<long, Dictionary<long, long>> dictionary2 =
      ...
      foreach (long groupID in list) {
        ...
        foreach (long stateID in dictionary1[groupID]) {
          ...
          this.StateGroups[groupID][stateID]
              .WriteHeader(bw, this.LongFormat, groupID, stateID);
        }
      ...
      foreach (var groupID in list) {
        void addCondition(ESD.Condition cond) {
          ...
          if (conditions[groupID]
              .Any<ESD.Condition>((Func<ESD.Condition, bool>) (c => cond == c)))
            return;
          // ISSUE: reference to a compiler-generated field
          // ISSUE: reference to a compiler-generated field
      ...
      // ISSUE: reference to a compiler-generated field
      bw.FillInt32("ConditionCount",
                   conditions.Values.Sum<List<ESD.Condition>>(
                       (Func<List<ESD.Condition>, int>)
                       (group => group.Count)));
      Dictionary<ESD.Condition, long> conditionOffsets =
          new Dictionary<ESD.Condition, long>();
      foreach (long groupID in list) {
        // ISSUE: reference to a compiler-generated field
      ...
      bw.FillInt32("CommandArgCount",
                   commandCallList.Sum<ESD.CommandCall>(
                       (Func<ESD.CommandCall, int>) (command
                                                          => command
                                                             .Arguments
                                                             .Count)));
      ...
      foreach (long groupID in list) {
        foreach (long stateID in dictionary1[groupID])
          num3 += this.StateGroups[groupID][stateID]
                      .WriteConditionOffsets(bw,
                                             this.LongFormat,
                                             groupID,
                                             stateID,
                                             position1,
      ...
      foreach (long groupID in list) {
        ...
        for (int index = 0; index < conditions[groupID].Count; ++index) {
          // ISSUE: reference to a compiler-generated field
          conditions[groupID][index]
              .WriteEvaluator(bw, this.LongFormat, groupID, index, position1);
        }
      }
      for (int index = 0; index < commandCallList.Count; ++index)
        commandCallList[index]
            .WriteBytecode(bw, this.LongFormat, index, position1);
    ...
    private Dictionary<long, ESD.State> TakeStates(
      ...
      Dictionary<long, ESD.State> dictionary =
          new Dictionary<long, ESD.State>(stateOffsets.Length);
    ...
    private static long[] ReadVarints(
        BinaryReaderEx br,
        bool longFormat,
        long count) {
      return longFormat
                 ? br.ReadInt64s((int) count)
                 : Array.ConvertAll<int, long>(br.ReadInt32s((int) count),
                                               (Converter<int, long>)
                                               (i => (long) i));
    }

    private static long AssertVarint(
        BinaryReaderEx br,
        bool longFormat,
        params long[] values) {
      return longFormat
                 ? br.AssertInt64(values)
                 : (long) br.AssertInt32(
                     Array.ConvertAll<long, int>(
                         values,
                         (Converter<long, int>) (l => (int) l)));
    }
    ...
    private static void ReserveVarint(
        BinaryWriterEx bw,
        bool longFormat,
        string name) {
      if (longFormat)
    ...
    private static void FillVarint(
        BinaryWriterEx bw,
        bool longFormat,
        string name,
        long value) {
    ...
    public class State {
      ...
      internal State(BinaryReaderEx br, bool longFormat, long dataStart) {
        ...
        for (int index = 0; (long) index < num3; ++index)
          this.EntryCommands.Add(
              new ESD.CommandCall(br, longFormat, dataStart));
    ...
    public class Condition {
      ...
      internal void GetStateAndConditions(
        ...
        this.stateOffset = -2L;
        this.Subconditions =
            new List<ESD.Condition>(this.conditionOffsets.Length);
        foreach (long conditionOffset in this.conditionOffsets)
      ...
      internal void WriteHeader(
        ...
        ESD.ReserveVarint(bw,
                          longFormat,
                          string.Format("Condition{0}-{1}:PassCommandsOffset",
                                        (object) groupID,
                                        (object) index));
        ESD.WriteVarint(bw, longFormat, (long) this.PassCommands.Count);
        ESD.ReserveVarint(bw,
                          longFormat,
                          string.Format("Condition{0}-{1}:ConditionsOffset",
                                        (object) groupID,
                                        (object) index));
        ESD.WriteVarint(bw, longFormat, (long) this.Subconditions.Count);
        ESD.ReserveVarint(bw,
                          longFormat,
                          string.Format("Condition{0}-{1}:EvaluatorOffset",
                                        (object) groupID,
                                        (object) index));
    ...
    public class CommandCall {
      ...
      internal void WriteHeader(BinaryWriterEx bw, bool longFormat, int index) {
        ...
        ESD.ReserveVarint(bw,
                          longFormat,
                          string.Format("Command{0}:ArgsOffset",
                                        (object) index));
      ...
      internal void WriteArgs(
        ...
        ESD.FillVarint(bw,
                       longFormat,
                       string.Format("Command{0}:ArgsOffset", (object) index),
                       bw.Position - dataStart);
        for (int index1 = 0; index1 < this.Arguments.Count; ++index1) {
          ESD.ReserveVarint(bw,
                            longFormat,
                            string.Format("Command{0}-{1}:BytecodeOffset",
                                          (object) index,
                                          (object) index1));
