...
namespace SoulsFormats
{
  ...
  public class BND4 : SoulsFile<BND4>, IBinder, IBND4
  {
    ...
    {
      List<BinderFileHeader> binderFileHeaderList = BND4.ReadHeader((IBND4) this, br);
      ...
      BinaryReaderEx br)
    {
      ...
      if (num5 != Binder.GetBND4FileHeaderSize(bnd.Format))
        throw new FormatException(string.Format("File header size for format {0} is expected to be 0x{1:X}, but was 0x{2:X}", (object) bnd.Format, (object) Binder.GetBND4FileHeaderSize(bnd.Format), (object) num5));
      List<BinderFileHeader> binderFileHeaderList = new List<BinderFileHeader>(capacity);
      for (int index = 0; index < capacity; ++index)
        binderFileHeaderList.Add(BinderFileHeader.ReadBinder4FileHeader(br, bnd.Format, bnd.BitBigEndian, bnd.Unicode));
    ...
    protected override void Write(BinaryWriterEx bw)
    {
      ...
      for (int index = 0; index < this.Files.Count; ++index)
        fileHeaders[index].WriteBinder4FileData(bw, bw, this.Format, index, this.Files[index].Bytes);
    }
      ...
      List<BinderFileHeader> fileHeaders)
    {
      ...
      for (int index = 0; index < fileHeaders.Count; ++index)
        fileHeaders[index].WriteBinder4FileHeader(bw, bnd.Format, bnd.BitBigEndian, index);
      for (int index = 0; index < fileHeaders.Count; ++index)
