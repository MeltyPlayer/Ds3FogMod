...
namespace SoulsFormats {
  ...
  public class TPF : SoulsFile<TPF>, IEnumerable<TPF.Texture>, IEnumerable {
    ...
    protected override void Read(BinaryReaderEx br) {
      ...
      br.BigEndian = this.Platform == TPF.TPFPlatform.Xbox360 ||
                     this.Platform == TPF.TPFPlatform.PS3;
      br.ReadInt32();
      ...
      for (int index = 0; index < capacity; ++index)
        this.Textures.Add(
            new TPF.Texture(br, this.Platform, this.Flag2, this.Encoding));
    }

    protected override void Write(BinaryWriterEx bw) {
      bw.BigEndian = this.Platform == TPF.TPFPlatform.Xbox360 ||
                     this.Platform == TPF.TPFPlatform.PS3;
    ...
    public class Texture {
      ...
      public Texture(
        ...
        this.Type = !dds.dwCaps2.HasFlag((System.Enum) DDS.DDSCAPS2.CUBEMAP)
                        ? (!dds.dwCaps2.HasFlag(
                               (System.Enum) DDS.DDSCAPS2.VOLUME)
                               ? TPF.TexType.Texture
                               : TPF.TexType.Volume)
                        : TPF.TexType.Cubemap;
          ...
          if (platform == TPF.TPFPlatform.Xbox360) {
            ...
            if (this.Flags2 == 1) {
              ...
              this.Unk28 = br.ReadSingle();
                     ...
                     platform == TPF.TPFPlatform.Xbone) {
        ...
        if (this.Flags1 == (byte) 2 || this.Flags1 == (byte) 3) {
          ...
          if (type != DCX.Type.DCP_EDGE)
            throw new NotImplementedException(
                string.Format(
                    "TPF compression is expected to be DCP_EDGE, but it was {0}",
                    (object) type));
        }
        switch (encoding) {
          case 0:
      ...
      internal void WriteName(BinaryWriterEx bw, int index, byte encoding) {
        bw.FillUInt32(string.Format("FileName{0}", (object) index),
                      (uint) bw.Position);
        switch (encoding) {
          case 0:
          case 2:
