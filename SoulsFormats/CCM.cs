    ...
    protected override void Read(BinaryReaderEx br) {
      ...
      if (Version == CCMVer.DemonsSouls || Version == CCMVer.DarkSouls1) {
        ...
        for (int i = 0; i < glyphCount; i++) {
          ...
          Vector2 uv1 = new Vector2(texRegion.X1 / (float) TexWidth,
                                    texRegion.Y1 / (float) TexHeight);
          Vector2 uv2 = new Vector2(texRegion.X2 / (float) TexWidth,
                                    texRegion.Y2 / (float) TexHeight);
          Glyphs[code] =
              new Glyph(uv1, uv2, preSpace, width, advance, texIndex);
        }
      }
    }
