...
namespace SoulsFormats
{
  ...
  public static class SFUtil
  {
    ...
    private static byte[] ds2SaveKey = SFUtil.ParseHexString("B7 FD 46 3E 4A 9C 11 02 DF 17 39 E5 F3 B2 A5 0F");
    private static byte[] scholarSaveKey = SFUtil.ParseHexString("59 9F 9B 69 96 40 A5 52 36 EE 2D 70 83 5E C7 44");
    private static byte[] ds3SaveKey = SFUtil.ParseHexString("FD 46 4D 69 5E 69 A3 9A 10 E3 19 A7 AC E8 B7 FA");
    ...
    public static string GuessExtension(byte[] bytes, bool bigEndian = false)
    {
      ...
      {
        BinaryReaderEx br = new BinaryReaderEx(bigEndian, (Stream) memoryStream);
        ...
        if (str2 == "AISD")
          ...
          str1 = ".evd";
        else if (br.Length >= 3L && br.GetASCII(0L, 3) == "FEV" || br.Length >= 16L && br.GetASCII(8L, 8) == "FEV FMT ")
          str1 = ".fev";
      ...
      {
        return br.Length >= 44L && Regex.IsMatch(br.GetASCII(12L, 32), "^[^\0]+\0 *$");
      }
      ...
      bool checkTdf(BinaryReaderEx br)
      {
        ...
        for (int index = 1; (long) index < br.Length; ++index)
        {
          if (br.GetASCII((long) index, 1) == "\"")
            return (long) index < br.Length - 2L && br.GetASCII((long) (index + 1), 2) == "\r\n";
        }
        return false;
      }
    }
    ...
    {
      return (byte) (((int) value & 1) << 7 | ((int) value & 2) << 5 | ((int) value & 4) << 3 | ((int) value & 8) << 1 | ((int) value & 16) >> 1 | ((int) value & 32) >> 3 | ((int) value & 64) >> 5 | ((int) value & 128) >> 7);
    }
    ...
    public static uint FromPathHash(string text)
    {
      ...
      if (!source.StartsWith("/"))
        source = "/" + source;
      return source.Aggregate<char, uint>(0U, (Func<uint, char, uint>) ((i, c) => i * 37U + (uint) c));
    }
    ...
    public static bool IsPrime(uint candidate)
    {
      ...
      if (candidate % 2U == 0U)
        return false;
      for (int index = 3; (long) (index * index) <= (long) candidate; index += 2)
      {
    ...
    {
      System.Text.RegularExpressions.Match match = SFUtil.timestampRx.Match(timestamp);
      if (!match.Success)
        throw new InvalidDataException("Unrecognized timestamp format.");
      return new DateTime(int.Parse(match.Groups[1].Value) + 2000, (int) match.Groups[2].Value[0] - 65, int.Parse(match.Groups[3].Value), (int) match.Groups[4].Value[0] - 65, int.Parse(match.Groups[5].Value), 0);
    }
    ...
    public static string DateToBinderTimestamp(DateTime dateTime)
    {
      ...
      return string.Format("{0:D2}{1}{2}{3}{4}", (object) num, (object) ch1, (object) day, (object) ch2, (object) minute).PadRight(8, char.MinValue);
    }
    ...
    public static byte[] ReadZlib(BinaryReaderEx br, int compressedSize)
    {
      ...
      int num2 = (int) br.AssertByte((byte) 1, (byte) 94, (byte) 156, (byte) 218);
      byte[] buffer = br.ReadBytes(compressedSize - 2);
      using (MemoryStream memoryStream1 = new MemoryStream())
      {
        using (MemoryStream memoryStream2 = new MemoryStream(buffer))
        {
          using (DeflateStream deflateStream = new DeflateStream((Stream) memoryStream2, CompressionMode.Decompress, true))
    ...
    public static byte[] DecryptSL2File(byte[] encrypted, byte[] key)
    {
      ...
      using (Aes aes = Aes.Create())
      {
        ...
        using (MemoryStream memoryStream1 = new MemoryStream(encrypted, 32, encrypted.Length - 32))
        {
    ...
    public static byte[] EncryptSL2File(byte[] decrypted, byte[] key)
    {
      using (Aes aes = Aes.Create())
      {
        ...
        {
          using (CryptoStream cryptoStream = new CryptoStream((Stream) memoryStream1, encryptor, CryptoStreamMode.Read))
          {
            using (MemoryStream memoryStream2 = new MemoryStream())
            {
              using (MD5 md5 = MD5.Create())
              {
                ...
                Buffer.BlockCopy((Array) md5.ComputeHash(buffer, 16, buffer.Length - 16), 0, (Array) buffer, 0, 16);
                return buffer;
              }
            }
          }
    ...
    public static BND4 DecryptDS3Regulation(string path)
    {
      ...
      return SoulsFile<BND4>.Read(SFUtil.DecryptByteArray(SFUtil.ds3RegulationKey, secret));
    }
    ...
    private static byte[] EncryptByteArray(byte[] key, byte[] secret)
    {
      using (MemoryStream memoryStream = new MemoryStream())
      {
        using (AesManaged aesManaged = new AesManaged())
        {
          ...
          using (CryptoStream cryptoStream = new CryptoStream((Stream) memoryStream, aesManaged.CreateEncryptor(key, iv), CryptoStreamMode.Write))
    ...
    private static byte[] DecryptByteArray(byte[] key, byte[] secret)
    {
      ...
      Buffer.BlockCopy((Array) secret, rgbIV.Length, (Array) buffer, 0, buffer.Length);
      using (MemoryStream memoryStream = new MemoryStream())
      {
        using (AesManaged aesManaged = new AesManaged())
        {
          ...
          aesManaged.KeySize = 256;
          aesManaged.BlockSize = 128;
          using (CryptoStream cryptoStream = new CryptoStream((Stream) memoryStream, aesManaged.CreateDecryptor(key, rgbIV), CryptoStreamMode.Write))
