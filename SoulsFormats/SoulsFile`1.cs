...
namespace SoulsFormats
{
  ...
  public abstract class SoulsFile<TFormat> where TFormat : SoulsFile<TFormat>, new()
  {
    ...
    {
      return bytes.Length != 0 && new TFormat().Is(SFUtil.GetDecompressedBR(new BinaryReaderEx(false, bytes), out DCX.Type _));
    }

    public static bool Is(string path)
    {
      using (FileStream fileStream = File.OpenRead(path))
        return fileStream.Length != 0L && new TFormat().Is(SFUtil.GetDecompressedBR(new BinaryReaderEx(false, (Stream) fileStream), out DCX.Type _));
    }
    ...
    public static TFormat Read(byte[] bytes)
    {
      ...
      BinaryReaderEx decompressedBr = SFUtil.GetDecompressedBR(br, out format.Compression);
    ...
    {
      return SoulsFile<TFormat>.IsRead(new BinaryReaderEx(false, bytes), out file);
    }

    public static bool IsRead(string path, out TFormat file)
    {
      using (FileStream fileStream = File.OpenRead(path))
        return SoulsFile<TFormat>.IsRead(new BinaryReaderEx(false, (Stream) fileStream), out file);
    }
