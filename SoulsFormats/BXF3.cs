...
namespace SoulsFormats
{
  ...
  public class BXF3 : IBinder, IBXF3
  {
    ...
    {
      return BXF3.IsBHD(SFUtil.GetDecompressedBR(new BinaryReaderEx(false, bytes), out DCX.Type _));
    }

    public static bool IsBHD(string path)
    {
      using (FileStream fileStream = File.OpenRead(path))
        return BXF3.IsBHD(SFUtil.GetDecompressedBR(new BinaryReaderEx(false, (Stream) fileStream), out DCX.Type _));
    }

    public static bool IsBDT(byte[] bytes)
    {
      return BXF3.IsBDT(SFUtil.GetDecompressedBR(new BinaryReaderEx(false, bytes), out DCX.Type _));
    }

    public static bool IsBDT(string path)
    {
      using (FileStream fileStream = File.OpenRead(path))
        return BXF3.IsBDT(SFUtil.GetDecompressedBR(new BinaryReaderEx(false, (Stream) fileStream), out DCX.Type _));
    }

    public static BXF3 Read(byte[] bhdBytes, byte[] bdtBytes)
    {
      return new BXF3(new BinaryReaderEx(false, bhdBytes), new BinaryReaderEx(false, bdtBytes));
    }

    public static BXF3 Read(byte[] bhdBytes, string bdtPath)
    {
      using (FileStream fileStream = File.OpenRead(bdtPath))
        return new BXF3(new BinaryReaderEx(false, bhdBytes), new BinaryReaderEx(false, (Stream) fileStream));
    }

    public static BXF3 Read(string bhdPath, byte[] bdtBytes)
    {
      using (FileStream fileStream = File.OpenRead(bhdPath))
        return new BXF3(new BinaryReaderEx(false, (Stream) fileStream), new BinaryReaderEx(false, bdtBytes));
    }
    ...
    public static BXF3 Read(string bhdPath, string bdtPath)
    {
      using (FileStream fileStream1 = File.OpenRead(bhdPath))
      {
        using (FileStream fileStream2 = File.OpenRead(bdtPath))
          return new BXF3(new BinaryReaderEx(false, (Stream) fileStream1), new BinaryReaderEx(false, (Stream) fileStream2));
      }
    }
    ...
    private BXF3(BinaryReaderEx bhdReader, BinaryReaderEx bdtReader)
    {
      ...
      List<BinderFileHeader> binderFileHeaderList = BXF3.ReadBHFHeader((IBXF3) this, bhdReader);
      ...
      BinaryReaderEx br)
    {
      ...
      for (int index = 0; index < capacity; ++index)
        binderFileHeaderList.Add(BinderFileHeader.ReadBinder3FileHeader(br, bxf.Format, bxf.BitBigEndian));
    ...
    private void Write(BinaryWriterEx bhdWriter, BinaryWriterEx bdtWriter)
    {
      ...
      for (int index = 0; index < this.Files.Count; ++index)
        fileHeaders[index].WriteBinder3FileData(bhdWriter, bdtWriter, this.Format, index, this.Files[index].Bytes);
    }
      ...
      List<BinderFileHeader> fileHeaders)
    {
      ...
      for (int index = 0; index < fileHeaders.Count; ++index)
        fileHeaders[index].WriteBinder3FileHeader(bw, bxf.Format, bxf.BitBigEndian, index);
      for (int index = 0; index < fileHeaders.Count; ++index)
