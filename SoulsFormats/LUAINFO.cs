    ...
    public class Goal
    {
      ...
      internal void Write(BinaryWriterEx bw, bool longFormat, int index)
      {
        ...
        if (longFormat)
        {
          ...
          bw.ReserveInt64(string.Format("LogicInterruptNameOffset{0}", (object) index));
        }
        else
        {
          bw.ReserveUInt32(string.Format("NameOffset{0}", (object) index));
          bw.ReserveUInt32(string.Format("LogicInterruptNameOffset{0}", (object) index));
          bw.WriteBoolean(this.BattleInterrupt);
          bw.WriteBoolean(this.LogicInterrupt);
          bw.WriteInt16((short) 0);
        }
      ...
      internal void WriteStrings(BinaryWriterEx bw, bool longFormat, int index)
      {
        if (longFormat)
        {
          bw.FillInt64(string.Format("NameOffset{0}", (object) index), bw.Position);
          bw.WriteUTF16(this.Name, true);
          if (this.LogicInterruptName == null)
          {
            bw.FillInt64(string.Format("LogicInterruptNameOffset{0}", (object) index), 0L);
          }
          else
          {
            bw.FillInt64(string.Format("LogicInterruptNameOffset{0}", (object) index), bw.Position);
            bw.WriteUTF16(this.LogicInterruptName, true);
          }
        }
        else
        {
          bw.FillUInt32(string.Format("NameOffset{0}", (object) index), (uint) bw.Position);
          bw.WriteShiftJIS(this.Name, true);
          if (this.LogicInterruptName == null)
          {
            bw.FillUInt32(string.Format("LogicInterruptNameOffset{0}", (object) index), 0U);
          }
          else
          {
            bw.FillUInt32(string.Format("LogicInterruptNameOffset{0}", (object) index), (uint) bw.Position);
