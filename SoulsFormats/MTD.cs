...
namespace SoulsFormats {
  ...
  public class MTD : SoulsFile<MTD> {
    ...
    private static string AssertMarkedString(
        BinaryReaderEx br,
        byte marker,
        string assert) {
      ...
      if (str != assert)
        throw new InvalidDataException(
            string.Format(
                "Read marked string: {0} | Expected: {1} | Ending position: 0x{2:X}",
                (object) str,
                (object) assert,
                (object) br.Position));
      return str;
    ...
    private static void WriteMarkedString(
        BinaryWriterEx bw,
        byte marker,
        string str) {
    ...
    public class Param {
      ...
      internal Param(BinaryReaderEx br) {
        ...
        this.Type = (MTD.ParamType) System.Enum.Parse(typeof(MTD.ParamType),
                                                      MTD.ReadMarkedString(
                                                          br,
                                                          (byte) 4),
                                                      true);
        br.AssertInt32(1);
      ...
      internal void Write(BinaryWriterEx bw) {
        ...
        if (this.Type == MTD.ParamType.Bool) {
          type = 4096;
          marker = (byte) 192;
        } else if (this.Type == MTD.ParamType.Int ||
                   this.Type == MTD.ParamType.Int2) {
          type = 4097;
          marker = (byte) 197;
        } else if (this.Type == MTD.ParamType.Float ||
                   this.Type == MTD.ParamType.Float2 ||
                   (this.Type == MTD.ParamType.Float3 ||
                    this.Type == MTD.ParamType.Float4)) {
          type = 4098;
          marker = (byte) 202;
        }
        ...
        int num = -1;
        if (this.Type == MTD.ParamType.Bool ||
            this.Type == MTD.ParamType.Int ||
            this.Type == MTD.ParamType.Float)
          num = 1;
        else if (this.Type == MTD.ParamType.Int2 ||
                 this.Type == MTD.ParamType.Float2)
          num = 2;
      ...
      public override string ToString() {
        if (this.Type == MTD.ParamType.Float2 ||
            this.Type == MTD.ParamType.Float3 ||
            this.Type == MTD.ParamType.Float4)
          return this.Name +
                 " = {" +
                 string.Join<float>(", ",
                                    (IEnumerable<float>) (float[]) this.Value) +
                 "}";
        return this.Type == MTD.ParamType.Int2
                   ? this.Name +
                     " = {" +
                     string.Join<int>(", ",
                                      (IEnumerable<int>) (int[]) this.Value) +
                     "}"
                   : string.Format("{0} = {1}", (object) this.Name, this.Value);
      }
    }
    ...
    public class Texture {
      ...
      internal Texture(BinaryReaderEx br) {
        ...
        if (block.Version == 3) {
          ...
          if (block.Version != 5)
            throw new InvalidDataException(
                string.Format(
                    "Texture block version is expected to be 3 or 5, but it was {0}.",
                    (object) block.Version));
          this.Extended = true;
        }
        ...
        if (this.Extended) {
          ...
          this.UnkFloats =
              new List<float>((IEnumerable<float>) br.ReadSingles(count));
        } else {
          this.Path = "";
          this.UnkFloats = new List<float>();
        }
      ...
      internal void Write(BinaryWriterEx bw) {
        MTD.Block block =
            MTD.Block.Write(bw, 8192, this.Extended ? 5 : 3, (byte) 163);
    ...
    private class Block {
      ...
      public static MTD.Block Read(
        ...
        int version = num2;
        byte marker = assertMarker.HasValue
                          ? MTD.AssertMarker(br, assertMarker.Value)
                          : MTD.ReadMarker(br);
        return new MTD.Block(position, length, type, version, marker);
      }

      public static MTD.Block Write(
          BinaryWriterEx bw,
          int type,
          int version,
          byte marker) {
      ...
      public void Finish(BinaryWriterEx bw) {
        ...
        bw.FillUInt32(string.Format("Block{0:X}", (object) this.Start),
                      this.Length);
      }
    }
  }
