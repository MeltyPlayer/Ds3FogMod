...
namespace SoulsFormats {
  ...
  public static class Binder {
    public static Binder.Format ReadFormat(
        BinaryReaderEx br,
        bool bitBigEndian) {
      ...
      return (bitBigEndian
                  ...
                  : (((int) num & 1) == 0
                         ...
                         : (((int) num & 128) == 0 ? 1 : 0))) ==
             0
                 ? (Binder.Format) SFUtil.ReverseBits(num)
                 : (Binder.Format) num;
    }
    ...
    public static void WriteFormat(
      ...
      byte num =
          (bitBigEndian ? 1 : (Binder.ForceBigEndian(format) ? 1 : 0)) != 0
              ? (byte) format
              : SFUtil.ReverseBits((byte) format);
      bw.WriteByte(num);
    }
    ...
    public static bool HasNames(Binder.Format format) {
      return (uint) (format & (Binder.Format.Names1 | Binder.Format.Names2)) >
             0U;
    }
    ...
    public static long GetBND4FileHeaderSize(Binder.Format format) {
      return (long) (16 +
                     (Binder.HasLongOffsets(format) ? 8 : 4) +
                     (Binder.HasCompression(format) ? 8 : 0) +
                     (Binder.HasIDs(format) ? 4 : 0) +
                     (Binder.HasNames(format) ? 4 : 0));
    }
    ...
    public static Binder.FileFlags ReadFileFlags(
      ...
      return num1 == 0
                 ? (Binder.FileFlags) SFUtil.ReverseBits(num2)
                 : (Binder.FileFlags) num2;
    }
