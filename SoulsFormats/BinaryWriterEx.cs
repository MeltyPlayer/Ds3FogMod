  ...
  public class BinaryWriterEx
  {
    ...
    public void Finish()
    {
      if (this.reservations.Count > 0)
        throw new InvalidOperationException("Not all reservations filled: " + string.Join(", ", (IEnumerable<string>) this.reservations.Keys));
      this.bw.Close();
    }
    ...
    public void WriteFixStrW(string text, int size, byte padding = 0)
    {
      ...
      for (int index = 0; index < size; ++index)
        buffer[index] = padding;
      byte[] numArray = !this.BigEndian ? SFEncoding.UTF16.GetBytes(text + "\0") : SFEncoding.UTF16BE.GetBytes(text + "\0");
      Array.Copy((Array) numArray, (Array) buffer, Math.Min(size, numArray.Length));
      this.bw.Write(buffer);
    }
