...
namespace SoulsFormats {
  internal static class BinderHashTable {
    ...
    public static void Write(BinaryWriterEx bw, List<BinderFileHeader> files) {
      uint num1 = 0;
      for (uint candidate = (uint) files.Count / 7U;
           candidate <= 100000U;
           ++candidate) {
      ...
      if (num1 == 0U)
        throw new InvalidOperationException(
            "Could not determine hash group count.");
      List<BinderHashTable.PathHash>[] pathHashListArray =
          new List<BinderHashTable.PathHash>[(int) num1];
      for (int index = 0; (long) index < (long) num1; ++index)
        pathHashListArray[index] = new List<BinderHashTable.PathHash>();
      for (int index = 0; index < files.Count; ++index) {
        BinderHashTable.PathHash pathHash =
            new BinderHashTable.PathHash(index, files[index].Name);
        uint num2 = pathHash.Hash % num1;
        pathHashListArray[(int) num2].Add(pathHash);
      }
      for (int index = 0; (long) index < (long) num1; ++index)
        pathHashListArray[index]
            .Sort((Comparison<BinderHashTable.PathHash>) ((ph1, ph2)
                                                               => ph1
                                                                  .Hash
                                                                  .CompareTo(
                                                                      ph2.Hash)
                                                         ));
      List<BinderHashTable.HashGroup> hashGroupList =
          new List<BinderHashTable.HashGroup>();
      List<BinderHashTable.PathHash> pathHashList1 =
          new List<BinderHashTable.PathHash>();
      int num3 = 0;
