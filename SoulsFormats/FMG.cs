...
namespace SoulsFormats
{
  ...
  public class FMG : SoulsFile<FMG>
  {
    ...
    protected override void Read(BinaryReaderEx br)
    {
      ...
      int num4 = (int) br.AssertByte(this.Version == FMG.FMGVersion.DemonsSouls ? byte.MaxValue : (byte) 0);
      ...
      for (int index1 = 0; index1 < capacity; ++index1)
      {
        ...
        for (int index2 = 0; index2 < num10 - num9 + 1; ++index2)
        {
          ...
          this.Entries.Add(new FMG.Entry(num9 + index2, offset != 0L ? br.GetUTF16(offset) : (string) null));
        }
        br.StepOut();
      }
    }
    ...
    protected override void Write(BinaryWriterEx bw)
    {
      ...
      bw.WriteByte((byte) 1);
      bw.WriteByte(this.Version == FMG.FMGVersion.DemonsSouls ? byte.MaxValue : (byte) 0);
      bw.WriteByte((byte) 0);
      bw.WriteByte((byte) 0);
      ...
      if (flag)
        ...
        bw.WriteInt32(0);
      int num = 0;
      this.Entries.Sort((Comparison<FMG.Entry>) ((e1, e2) => e1.ID.CompareTo(e2.ID)));
      for (int index = 0; index < this.Entries.Count; ++index)
      {
        bw.WriteInt32(index);
        bw.WriteInt32(this.Entries[index].ID);
        while (index < this.Entries.Count - 1 && this.Entries[index + 1].ID == this.Entries[index].ID + 1)
          ++index;
        bw.WriteInt32(this.Entries[index].ID);
        if (flag)
          bw.WriteInt32(0);
      ...
      for (int index = 0; index < this.Entries.Count; ++index)
      {
        ...
        if (flag)
          bw.FillInt64(string.Format("StringOffset{0}", (object) index), text == null ? 0L : bw.Position);
        else
          bw.FillInt32(string.Format("StringOffset{0}", (object) index), text == null ? 0 : (int) bw.Position);
        if (text != null)
    ...
    public string this[int id]
    {
      get
      {
        return this.Entries.Find((Predicate<FMG.Entry>) (entry => entry.ID == id))?.Text;
      }
      set
      {
        if (this.Entries.Any<FMG.Entry>((Func<FMG.Entry, bool>) (entry => entry.ID == id)))
          this.Entries.Find((Predicate<FMG.Entry>) (entry => entry.ID == id)).Text = value;
        else
    ...
    public class Entry
    {
      ...
      {
        return string.Format("{0}: {1}", (object) this.ID, (object) (this.Text ?? "<null>"));
      }
    }
