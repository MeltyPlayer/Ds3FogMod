...
namespace SoulsFormats
{
  ...
  public class FLVER2 : SoulsFile<FLVER2>, IFlver
  {
    ...
    protected override void Read(BinaryReaderEx br)
    {
      ...
      this.Header.BigEndian = (br.AssertASCII("L\0", "B\0") == "B\0" ? 1 : 0) != 0;
      br.BigEndian = this.Header.BigEndian;
      this.Header.Version = br.AssertInt32(131077, 131081, 131084, 131085, 131086, 131087, 131088, 131091, 131092, 131094, 131098);
      ...
      for (int index = 0; index < capacity2; ++index)
        this.Materials.Add(new FLVER2.Material(br, this.Header, this.GXLists, gxListIndices));
      ...
      for (int index = 0; index < capacity6; ++index)
        items1.Add(new FLVER2.FaceSet(br, this.Header, headerIndexSize, dataOffset));
      List<FLVER2.VertexBuffer> items2 = new List<FLVER2.VertexBuffer>(capacity5);
      ...
      Dictionary<int, FLVER2.Texture> textureDict = SFUtil.Dictionize<FLVER2.Texture>(items3);
      ...
      Dictionary<int, FLVER2.FaceSet> faceSetDict = SFUtil.Dictionize<FLVER2.FaceSet>(items1);
      Dictionary<int, FLVER2.VertexBuffer> vertexBufferDict = SFUtil.Dictionize<FLVER2.VertexBuffer>(items2);
    ...
    protected override void Write(BinaryWriterEx bw)
    {
      ...
      bw.WriteInt32(this.Meshes.Sum<FLVER2.Mesh>((Func<FLVER2.Mesh, int>) (m => m.VertexBuffers.Count)));
      ...
      foreach (FLVER2.Mesh mesh in this.Meshes)
      {
        bool allowPrimitiveRestarts = mesh.Vertices.Count < (int) ushort.MaxValue;
      ...
      bw.WriteInt32(this.Meshes.Sum<FLVER2.Mesh>((Func<FLVER2.Mesh, int>) (m => m.FaceSets.Count)));
      bw.WriteInt32(this.BufferLayouts.Count);
      bw.WriteInt32(this.Materials.Sum<FLVER2.Material>((Func<FLVER2.Material, int>) (m => m.Textures.Count)));
      ...
      foreach (FLVER2.Mesh mesh in this.Meshes)
      {
        for (int bufferIndex = 0; bufferIndex < mesh.VertexBuffers.Count; ++bufferIndex)
          mesh.VertexBuffers[bufferIndex].Write(bw, this.Header, num3 + bufferIndex, bufferIndex, this.BufferLayouts, mesh.Vertices.Count);
      ...
      {
        bw.FillInt32(string.Format("MeshFaceSetIndices{0}", (object) index1), (int) bw.Position);
        for (int index2 = 0; index2 < this.Meshes[index1].FaceSets.Count; ++index2)
      ...
      for (int index1 = 0; index1 < this.Meshes.Count; ++index1)
      {
        bw.FillInt32(string.Format("MeshVertexBufferIndices{0}", (object) index1), (int) bw.Position);
        for (int index2 = 0; index2 < this.Meshes[index1].VertexBuffers.Count; ++index2)
      ...
      for (int index1 = 0; index1 < this.Materials.Count; ++index1)
      {
        ...
        for (int index2 = 0; index2 < material.Textures.Count; ++index2)
          material.Textures[index2].WriteStrings(bw, this.Header, num6 + index2);
      ...
      for (int index1 = 0; index1 < this.Meshes.Count; ++index1)
      {
        ...
        for (int index2 = 0; index2 < mesh.FaceSets.Count; ++index2)
        {
          ...
          bw.Pad(align);
          mesh.FaceSets[index2].WriteVertices(bw, indexSize, num7 + index2, position2);
        }
        ...
        {
          bw.Pad(align);
          mesh.VertexBuffers[index2].WriteBuffer(bw, num8 + index2, this.BufferLayouts, mesh.Vertices, position2, this.Header);
        }
        foreach (FLVER.Vertex vertex in mesh.Vertices)
          vertex.FinishWrite();
    ...
    public class BufferLayout : List<FLVER.LayoutMember>
    {
      ...
      {
        bw.FillInt32(string.Format("VertexStructLayout{0}", (object) index), (int) bw.Position);
        int structOffset = 0;
        foreach (FLVER.LayoutMember layoutMember in (List<FLVER.LayoutMember>) this)
        {
    ...
    public class FaceSet
    {
        ...
        int dataOffset)
      {
        ...
        {
          if (num3 != 32)
            throw new NotImplementedException(string.Format("Unsupported index size: {0}", (object) num3));
          this.Indices = new List<int>((IEnumerable<int>) br.GetInt32s((long) (dataOffset + num2), num1));
        }
      }

      internal void Write(BinaryWriterEx bw, FLVER2.FLVERHeader header, int indexSize, int index)
      {
      ...
      {
        bw.FillInt32(string.Format("FaceSetVertices{0}", (object) index), (int) bw.Position - dataStart);
        switch (indexSize)
        {
          case 16:
            using (List<int>.Enumerator enumerator = this.Indices.GetEnumerator())
            {
        ...
        ref int totalFaceCount)
      {
        if (this.TriangleStrip)
        {
          for (int index1 = 0; index1 < this.Indices.Count - 2; ++index1)
          {
            ...
            if (!allowPrimitiveRestarts || index2 != (int) ushort.MaxValue && index3 != (int) ushort.MaxValue && index4 != (int) ushort.MaxValue)
            {
              ++totalFaceCount;
              if ((this.Flags & FLVER2.FaceSet.FSFlags.MotionBlur) == FLVER2.FaceSet.FSFlags.None && index2 != index3 && (index3 != index4 && index4 != index2))
                ++trueFaceCount;
            }
          }
        }
      ...
      public List<int> Triangulate(bool allowPrimitiveRestarts, bool includeDegenerateFaces = false)
      {
        ...
        for (int index1 = 0; index1 < this.Indices.Count - 2; ++index1)
        {
          ...
          {
            if (includeDegenerateFaces || index2 != index3 && index3 != index4 && index4 != index2)
            {
              if (flag)
              {
                intList.Add(index4);
    ...
    public class GXItem
    {
      ...
      {
        this.ID = header.Version > 131088 ? br.ReadFixStr(4) : br.ReadInt32().ToString();
    ...
    public class Material : IFlverMaterial
    {
      ...
      internal void TakeTextures(Dictionary<int, FLVER2.Texture> textureDict)
      {
        ...
        for (int textureIndex = this.textureIndex; textureIndex < this.textureIndex + this.textureCount; ++textureIndex)
        {
          if (!textureDict.ContainsKey(textureIndex))
            throw new NotSupportedException("Texture not found or already taken: " + (object) textureIndex);
      ...
      internal void FillGXOffset(BinaryWriterEx bw, int index, List<int> gxOffsets)
      {
        if (this.GXIndex == -1)
          ...
          bw.FillInt32(string.Format("GXOffset{0}", (object) index), gxOffsets[this.GXIndex]);
      }

      internal void WriteTextures(BinaryWriterEx bw, int index, int textureIndex)
      {
        bw.FillInt32(string.Format("TextureIndex{0}", (object) index), textureIndex);
        for (int index1 = 0; index1 < this.Textures.Count; ++index1)
          this.Textures[index1].Write(bw, textureIndex + index1);
      }

      internal void WriteStrings(BinaryWriterEx bw, FLVER2.FLVERHeader header, int index)
      {
        bw.FillInt32(string.Format("MaterialName{0}", (object) index), (int) bw.Position);
        if (header.Unicode)
          bw.WriteUTF16(this.Name, true);
        else
          bw.WriteShiftJIS(this.Name, true);
        bw.FillInt32(string.Format("MaterialMTD{0}", (object) index), (int) bw.Position);
        if (header.Unicode)
    ...
    public class Mesh : IFlverMesh
    {
      ...
      internal Mesh(BinaryReaderEx br, FLVER2.FLVERHeader header)
      {
        ...
        this.BoneIndices = new List<int>((IEnumerable<int>) br.GetInt32s((long) num5, count1));
      ...
      internal void TakeFaceSets(Dictionary<int, FLVER2.FaceSet> faceSetDict)
      {
        ...
        foreach (int faceSetIndex in this.faceSetIndices)
        {
          if (!faceSetDict.ContainsKey(faceSetIndex))
            throw new NotSupportedException("Face set not found or already taken: " + (object) faceSetIndex);
      ...
      {
        this.VertexBuffers = new List<FLVER2.VertexBuffer>(this.vertexBufferIndices.Length);
        foreach (int vertexBufferIndex in this.vertexBufferIndices)
        {
          if (!vertexBufferDict.ContainsKey(vertexBufferIndex))
            throw new NotSupportedException("Vertex buffer not found or already taken: " + (object) vertexBufferIndex);
        ...
        List<FLVER.LayoutSemantic> layoutSemanticList = new List<FLVER.LayoutSemantic>();
        foreach (FLVER2.VertexBuffer vertexBuffer in this.VertexBuffers)
        {
          foreach (FLVER.LayoutMember layoutMember in (List<FLVER.LayoutMember>) layouts[vertexBuffer.LayoutIndex])
          {
            if (layoutMember.Semantic != FLVER.LayoutSemantic.UV && layoutMember.Semantic != FLVER.LayoutSemantic.Tangent && (layoutMember.Semantic != FLVER.LayoutSemantic.VertexColor && layoutMember.Semantic != FLVER.LayoutSemantic.Position) && layoutMember.Semantic != FLVER.LayoutSemantic.Normal)
            {
      ...
      internal void ReadVertices(
        ...
        FLVER2.FLVERHeader header)
      {
        IEnumerable<FLVER.LayoutMember> source = layouts.SelectMany<FLVER2.BufferLayout, FLVER.LayoutMember>((Func<FLVER2.BufferLayout, IEnumerable<FLVER.LayoutMember>>) (l => (IEnumerable<FLVER.LayoutMember>) l));
        int uvCapacity = source.Where<FLVER.LayoutMember>((Func<FLVER.LayoutMember, bool>) (m => m.Semantic == FLVER.LayoutSemantic.UV)).Count<FLVER.LayoutMember>();
        int tangentCapacity = source.Where<FLVER.LayoutMember>((Func<FLVER.LayoutMember, bool>) (m => m.Semantic == FLVER.LayoutSemantic.Tangent)).Count<FLVER.LayoutMember>();
        int colorCapacity = source.Where<FLVER.LayoutMember>((Func<FLVER.LayoutMember, bool>) (m => m.Semantic == FLVER.LayoutSemantic.VertexColor)).Count<FLVER.LayoutMember>();
        int vertexCount = this.VertexBuffers[0].VertexCount;
        this.Vertices = new List<FLVER.Vertex>(vertexCount);
        for (int index = 0; index < vertexCount; ++index)
          this.Vertices.Add(new FLVER.Vertex(uvCapacity, tangentCapacity, colorCapacity));
        foreach (FLVER2.VertexBuffer vertexBuffer in this.VertexBuffers)
          vertexBuffer.ReadBuffer(br, layouts, this.Vertices, dataOffset, header);
      }
      ...
      internal void Write(BinaryWriterEx bw, int index)
      {
        ...
        bw.ReserveInt32(string.Format("MeshVertexBufferIndices{0}", (object) index));
      }

      internal void WriteBoundingBox(BinaryWriterEx bw, int index, FLVER2.FLVERHeader header)
      {
        ...
        {
          bw.FillInt32(string.Format("MeshBoundingBox{0}", (object) index), (int) bw.Position);
      ...
      internal void WriteBoneIndices(BinaryWriterEx bw, int index, int boneIndicesStart)
      {
        ...
        {
          bw.FillInt32(string.Format("MeshBoneIndices{0}", (object) index), boneIndicesStart);
        }
        else
        {
          bw.FillInt32(string.Format("MeshBoneIndices{0}", (object) index), (int) bw.Position);
          bw.WriteInt32s((IList<int>) this.BoneIndices);
        }
      }

      public List<FLVER.Vertex[]> GetFaces(FLVER2.FaceSet.FSFlags fsFlags = FLVER2.FaceSet.FSFlags.None)
      {
        if (this.FaceSets.Count == 0)
          return new List<FLVER.Vertex[]>();
        List<int> intList = (this.FaceSets.Find((Predicate<FLVER2.FaceSet>) (fs => fs.Flags == fsFlags)) ?? this.FaceSets[0]).Triangulate(this.Vertices.Count < (int) ushort.MaxValue, false);
        List<FLVER.Vertex[]> vertexArrayList = new List<FLVER.Vertex[]>(intList.Count);
    ...
    public class Texture : IFlverTexture
    {
      ...
      internal void WriteStrings(BinaryWriterEx bw, FLVER2.FLVERHeader header, int index)
      {
        bw.FillInt32(string.Format("TexturePath{0}", (object) index), (int) bw.Position);
        if (header.Unicode)
          bw.WriteUTF16(this.Path, true);
        else
          bw.WriteShiftJIS(this.Path, true);
        bw.FillInt32(string.Format("TextureType{0}", (object) index), (int) bw.Position);
        if (header.Unicode)
    ...
    public class VertexBuffer
    {
      ...
      {
        FLVER2.BufferLayout layout = layouts[this.LayoutIndex];
        bw.FillInt32(string.Format("VertexBufferOffset{0}", (object) index), (int) bw.Position - dataStart);
        float uvFactor = 1024f;
        if (header.Version >= 131087)
          uvFactor = 2048f;
