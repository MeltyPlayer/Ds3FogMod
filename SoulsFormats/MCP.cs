...
namespace SoulsFormats
{
  ...
  public class MCP : SoulsFile<MCP>
  {
    ...
    {
      if (!SoulsFile<MCP>.ValidateNull((object) this.Rooms, "Rooms may not be null.", out ex))
        return false;
      for (int index1 = 0; index1 < this.Rooms.Count; ++index1)
      {
        MCP.Room room = this.Rooms[index1];
        if (!SoulsFile<MCP>.ValidateNull((object) room, string.Format("{0}[{1}]: {2} may not be null.", (object) "Rooms", (object) index1, (object) "Room"), out ex) || !SoulsFile<MCP>.ValidateNull((object) room.ConnectedRoomIndices, string.Format("{0}[{1}]: {2} may not be null.", (object) "Rooms", (object) index1, (object) "ConnectedRoomIndices"), out ex))
          return false;
        for (int index2 = 0; index2 < room.ConnectedRoomIndices.Count; ++index2)
        {
          int connectedRoomIndex = room.ConnectedRoomIndices[index2];
          if (!SoulsFile<MCP>.ValidateIndex((long) this.Rooms.Count, (long) connectedRoomIndex, string.Format("{0}[{1}].{2}[{3}]: Index out of range: {4}", (object) "Rooms", (object) index1, (object) "ConnectedRoomIndices", (object) index2, (object) connectedRoomIndex), out ex))
            return false;
        }
      }
      ex = (Exception) null;
    ...
    public class Room
    {
      ...
      internal Room(BinaryReaderEx br)
      {
        ...
        this.ConnectedRoomIndices = new List<int>((IEnumerable<int>) br.GetInt32s((long) num, count));
      }
