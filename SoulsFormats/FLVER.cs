  ...
  public static class FLVER {
    public class Bone {
      ...
      internal void WriteStrings(BinaryWriterEx bw, bool unicode, int index) {
        bw.FillInt32(string.Format("BoneNameOffset{0}", (object) index),
                     (int) bw.Position);
        if (unicode)
          ...
          switch (this.Type) {
            ...
            default:
              throw new NotImplementedException(
                  string.Format("No size defined for buffer layout type: {0}",
                                (object) this.Type));
          }
        }
      }
    ...
    public class Vertex {
      ...
      public Vertex(FLVER.Vertex clone) {
        ...
        this.Tangents =
            new List<Vector4>((IEnumerable<Vector4>) clone.Tangents);
        this.Bitangent = clone.Bitangent;
        this.Colors =
            new List<FLVER.VertexColor>(
                (IEnumerable<FLVER.VertexColor>) clone.Colors);
      }

      internal void PrepareWrite() {
        this.uvQueue = new Queue<Vector3>((IEnumerable<Vector3>) this.UVs);
        this.tangentQueue =
            new Queue<Vector4>((IEnumerable<Vector4>) this.Tangents);
        this.colorQueue =
            new Queue<FLVER.VertexColor>(
                (IEnumerable<FLVER.VertexColor>) this.Colors);
      }
      ...
      internal void Read(
          BinaryReaderEx br,
          List<FLVER.LayoutMember> layout,
          float uvFactor) {
          ...
          if (layoutMember.Semantic == FLVER.LayoutSemantic.Position) {
            if (layoutMember.Type == FLVER.LayoutType.Float3) {
              ...
              if (layoutMember.Type != FLVER.LayoutType.Float4)
                throw new NotImplementedException(
                    string.Format("Read not implemented for {0} {1}.",
                                  (object) layoutMember.Type,
                                  (object) layoutMember.Semantic));
              this.Position = br.ReadVector3();
              double num = (double) br.AssertSingle(new float[1]);
            }
          } else if (layoutMember.Semantic ==
                     FLVER.LayoutSemantic.BoneWeights) {
            if (layoutMember.Type == FLVER.LayoutType.Byte4A) {
              for (int index = 0; index < 4; ++index)
                this.BoneWeights[index] =
                    (float) br.ReadSByte() / (float) sbyte.MaxValue;
            } else if (layoutMember.Type == FLVER.LayoutType.Byte4C) {
              for (int index = 0; index < 4; ++index)
                this.BoneWeights[index] =
                    (float) br.ReadByte() / (float) byte.MaxValue;
            } else if (layoutMember.Type == FLVER.LayoutType.UVPair) {
              for (int index = 0; index < 4; ++index)
                this.BoneWeights[index] =
                    (float) br.ReadInt16() / (float) short.MaxValue;
            } else {
              ...
              for (int index = 0; index < 4; ++index)
                this.BoneWeights[index] =
                    (float) br.ReadInt16() / (float) short.MaxValue;
                     ...
                     FLVER.LayoutSemantic.BoneIndices) {
            ...
            if (layoutMember.Type == FLVER.LayoutType.Float3)
              ...
              if ((double) num != (double) this.NormalW)
                throw new InvalidDataException(
                    string.Format("Float4 Normal W was not a whole number: {0}",
                                  (object) num));
            ...
            if (layoutMember.Type == FLVER.LayoutType.Float2)
              ...
              this.UVs.Add(new Vector3((float) br.ReadInt16(),
                                       (float) br.ReadInt16(),
                                       0.0f) /
                           uvFactor);
            else if (layoutMember.Type == FLVER.LayoutType.Byte4B)
      ...
      private static float ReadByteNorm(BinaryReaderEx br) {
        return (float) ((int) br.ReadByte() - (int) sbyte.MaxValue) /
               (float) sbyte.MaxValue;
      }

      private static Vector3 ReadByteNormXYZ(BinaryReaderEx br) {
        return new Vector3(FLVER.Vertex.ReadByteNorm(br),
                           FLVER.Vertex.ReadByteNorm(br),
                           FLVER.Vertex.ReadByteNorm(br));
      }

      private static Vector4 ReadByteNormXYZW(BinaryReaderEx br) {
        return new Vector4(FLVER.Vertex.ReadByteNorm(br),
                           FLVER.Vertex.ReadByteNorm(br),
                           FLVER.Vertex.ReadByteNorm(br),
                           FLVER.Vertex.ReadByteNorm(br));
      }
      ...
      private static Vector3 ReadShortNormXYZ(BinaryReaderEx br) {
        return new Vector3(FLVER.Vertex.ReadShortNorm(br),
                           FLVER.Vertex.ReadShortNorm(br),
                           FLVER.Vertex.ReadShortNorm(br));
      }

      private static Vector4 ReadShortNormXYZW(BinaryReaderEx br) {
        return new Vector4(FLVER.Vertex.ReadShortNorm(br),
                           FLVER.Vertex.ReadShortNorm(br),
                           FLVER.Vertex.ReadShortNorm(br),
                           FLVER.Vertex.ReadShortNorm(br));
      }
      ...
      private static float ReadUShortNorm(BinaryReaderEx br) {
        return (float) ((int) br.ReadUInt16() - (int) short.MaxValue) /
               (float) short.MaxValue;
      }

      private static Vector3 ReadUShortNormXYZ(BinaryReaderEx br) {
        return new Vector3(FLVER.Vertex.ReadUShortNorm(br),
                           FLVER.Vertex.ReadUShortNorm(br),
                           FLVER.Vertex.ReadUShortNorm(br));
      }
        ...
        foreach (FLVER.LayoutMember layoutMember in layout) {
          if (layoutMember.Semantic == FLVER.LayoutSemantic.Position) {
            if (layoutMember.Type == FLVER.LayoutType.Float3) {
              ...
              if (layoutMember.Type != FLVER.LayoutType.Float4)
                throw new NotImplementedException(
                    string.Format("Write not implemented for {0} {1}.",
                                  (object) layoutMember.Type,
                                  (object) layoutMember.Semantic));
              bw.WriteVector3(this.Position);
              bw.WriteSingle(0.0f);
            ...
            if (layoutMember.Type == FLVER.LayoutType.Byte4A) {
              for (int index = 0; index < 4; ++index)
                bw.WriteSByte((sbyte) Math.Round(
                                  (double) this.BoneWeights[index] *
                                  (double) sbyte.MaxValue));
            } else if (layoutMember.Type == FLVER.LayoutType.Byte4C) {
              for (int index = 0; index < 4; ++index)
                bw.WriteByte((byte) Math.Round(
                                 (double) this.BoneWeights[index] *
                                 (double) byte.MaxValue));
            } else if (layoutMember.Type == FLVER.LayoutType.UVPair) {
              for (int index = 0; index < 4; ++index)
                bw.WriteInt16((short) Math.Round(
                                  (double) this.BoneWeights[index] *
      ...
      private static void WriteByteNorm(BinaryWriterEx bw, float value) {
        bw.WriteByte((byte) Math.Round(
                         (double) value * (double) sbyte.MaxValue +
                         (double) sbyte.MaxValue));
      }
      ...
      private static void WriteSByteNorm(BinaryWriterEx bw, float value) {
        bw.WriteSByte(
            (sbyte) Math.Round((double) value * (double) sbyte.MaxValue));
      }
      ...
      private static void WriteShortNorm(BinaryWriterEx bw, float value) {
        bw.WriteInt16(
            (short) Math.Round((double) value * (double) short.MaxValue));
      }
      ...
      private static void WriteUShortNorm(BinaryWriterEx bw, float value) {
        bw.WriteUInt16((ushort) Math.Round(
                           (double) value * (double) short.MaxValue +
                           (double) short.MaxValue));
      }
          ...
          switch (i) {
            ...
            default:
              throw new IndexOutOfRangeException(
                  string.Format(
                      "Index ({0}) was out of range. Must be non-negative and less than 4.",
                      (object) i));
          }
        }
        set {
    ...
    public struct VertexColor {
      ...
      internal static FLVER.VertexColor ReadByteARGB(BinaryReaderEx br) {
        ...
        int num6 = (int) num3;
        int num7 = (int) num4;
        return new FLVER.VertexColor((byte) num1,
                                     (byte) num5,
                                     (byte) num6,
                                     (byte) num7);
      }
      ...
      internal void WriteByteARGB(BinaryWriterEx bw) {
        bw.WriteByte(
            (byte) Math.Round((double) this.A * (double) byte.MaxValue));
        bw.WriteByte(
            (byte) Math.Round((double) this.R * (double) byte.MaxValue));
        bw.WriteByte(
            (byte) Math.Round((double) this.G * (double) byte.MaxValue));
        bw.WriteByte(
            (byte) Math.Round((double) this.B * (double) byte.MaxValue));
      }
