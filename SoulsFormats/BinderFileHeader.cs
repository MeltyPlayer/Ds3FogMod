...
namespace SoulsFormats
{
  ...
  public class BinderFileHeader
  {
      ...
      bool bitBigEndian)
    {
      ...
      long dataOffset = !Binder.HasLongOffsets(format) ? (long) br.ReadUInt32() : br.ReadInt64();
      int id = -1;
      ...
      if (Binder.HasCompression(format))
        num6 = br.ReadInt32();
      return new BinderFileHeader(flags, id, name, (long) num4, (long) num6, dataOffset);
    }
      ...
      bool unicode)
    {
      ...
      if (Binder.HasNames(format))
      {
        ...
        name = !unicode ? br.GetShiftJIS((long) num4) : br.GetUTF16((long) num4);
      }
    ...
    internal BinderFile ReadFileData(BinaryReaderEx br)
    {
      ...
      return new BinderFile(this.Flags, this.ID, this.Name, !Binder.IsCompressed(this.Flags) ? br.GetBytes(this.DataOffset, (int) this.CompressedSize) : DCX.Decompress(br.GetBytes(this.DataOffset, (int) this.CompressedSize), out type))
      {
      ...
      int index)
    {
      ...
      if (Binder.HasCompression(format))
        bw.ReserveInt64(string.Format("FileUncompressedSize{0}", (object) index));
      if (Binder.HasLongOffsets(format))
        bw.ReserveInt64(string.Format("FileDataOffset{0}", (object) index));
      else
        bw.ReserveUInt32(string.Format("FileDataOffset{0}", (object) index));
      ...
      byte[] bytes)
    {
      ...
      bwHeader.FillInt32(string.Format("FileCompressedSize{0}", (object) index), (int) this.CompressedSize);
      if (Binder.HasCompression(format))
        bwHeader.FillInt32(string.Format("FileUncompressedSize{0}", (object) index), (int) this.UncompressedSize);
      if (Binder.HasLongOffsets(format))
        bwHeader.FillInt64(string.Format("FileDataOffset{0}", (object) index), this.DataOffset);
      else
        bwHeader.FillUInt32(string.Format("FileDataOffset{0}", (object) index), (uint) this.DataOffset);
    }
    ...
    {
      this.WriteFileData(bwData, bytes);
      bwHeader.FillInt64(string.Format("FileCompressedSize{0}", (object) index), this.CompressedSize);
      if (Binder.HasCompression(format))
        bwHeader.FillInt64(string.Format("FileUncompressedSize{0}", (object) index), this.UncompressedSize);
      if (Binder.HasLongOffsets(format))
        bwHeader.FillInt64(string.Format("FileDataOffset{0}", (object) index), this.DataOffset);
      else
        bwHeader.FillUInt32(string.Format("FileDataOffset{0}", (object) index), (uint) this.DataOffset);
    ...
    internal void WriteFileName(BinaryWriterEx bw, Binder.Format format, bool unicode, int index)
    {
      if (!Binder.HasNames(format))
        return;
      bw.FillInt32(string.Format("FileNameOffset{0}", (object) index), (int) bw.Position);
      if (unicode)
